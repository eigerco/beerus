...
async fn main() -> Result<()> {
    ...
    let config = Config {
        ...
        starknet_rpc: format!(
            "https://starknet-mainnet.g.alchemy.com/v2/{api_key}"
        ),
        ..Default::default()
    };
    ...
    let calldata = FunctionCall {
        contract_address: FieldElement::from_str(starkgate_addr)?,
        entry_point_selector: FieldElement::from_str(name_selector)?,
        calldata: vec![],
    };
