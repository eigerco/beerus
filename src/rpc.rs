...
mod tests {
    ...
    fn make_state(state: (u64, &str)) -> ClientState {
        ClientState {
            block_number: state.0,
            block_hash: Felt::try_new(state.1).unwrap(),
    ...
    fn get_block_with_tx_hashes_response(state: (u64, &str)) -> String {
    ...
    #[tokio::test]
    async fn resolve_block_by_hash_success() {
    ...
    async fn resolve_block_id_hash_success() {
        ...
        assert!(result.is_ok());
        let (returned_block, _) = result.unwrap();
        assert!(eq(&returned_block, &block));
    }

    // TODO Test case where requested block hash is the same as current state
    //
    // TODO Test case where requested block hash is not valid, response??
    //
    // TODO Malicious hash return, same hash, wrong number
    //
    // TODO Update with state roots, compare values
}
